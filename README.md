...
* **Polymorphic Output:** The encoder selects byte offsets randomly. With enough entropy in the dictionary, multiple executions can produce unique arrays, making static string or byte-sequence signatures impossible.
...
* **Obfuscation, not Injection:** The core of this project is the obfuscation and reconstruction logic. The provided loaders use standard, well-known APIs and injection techniques (e.g., `CreateRemoteThread`, module stomping, dynamic function resolution).
* **Educational Purpose:** This project does not introduce new process injection primitives. It is designed to demonstrate how Living off the Land (LotL) approach can be applied to bypass static signature detection of shellcode.
* **Modular Design:** The reconstruction logic is designed to be injection-agnostic. It relies on a simple for loop which can be customized and used with any execution technique, including Process Hollowing, APC Injection, or Module Overloading.
...
Run the helper script to map your shellcode to a target dictionary file. This example utilizes **Windows CHM files** (Compiled HTML Help) for several strategic reasons:
...
- [ ] **Sleep Masking:** Create a function to restore obfuscated data when the process is inactive to hide from memory forensic tools.
